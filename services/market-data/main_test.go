...
func TestCorrelationIDPreserved(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodGet, "/health", nil)
	req.Header.Set(httpkit.CorrelationHeader, cid)
	rec := httptest.NewRecorder()
	testRouter().ServeHTTP(rec, req)

	if got := rec.Header().Get(httpkit.CorrelationHeader); got != cid {
		t.Fatalf("expected correlation id %q to be preserved, got %q", cid, got)
	}
}
